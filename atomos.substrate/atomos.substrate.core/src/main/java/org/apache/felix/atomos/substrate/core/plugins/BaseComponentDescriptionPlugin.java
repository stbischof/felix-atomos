...
import org.apache.felix.atomos.substrate.api.plugin.ReferenceDescription;

public class BaseComponentDescriptionPlugin implements ComponentMetaDataPlugin<PluginConfigBase>
{
            ...
            if (c.references() != null && !c.references().isEmpty())
            {
                for (ReferenceDescription r : c.references())
                {
                    ...
                    Optional.ofNullable(r.interfaceName()).ifPresent(
                        (i) -> context.addClass(i));
                }
            }
            boolean foundConstructor = false;
            if (c.numberOfConstructorParameters() == 0)
            {
                context.addConstructorDefault(clazz);
            ...
            else
            {
                for (Constructor<?> constructor : clazz.getConstructors())
                {
                    ...
                    if (match)
                    {
                        ...
                        context.addConstructor(clazz, ps);
                        foundConstructor = true;
                        break;
                    }
                }
                if (!foundConstructor)
                {
                    context.addConstructorAllPublic(clazz);
                }
            }
